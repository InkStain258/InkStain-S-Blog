...
export default function WeatherWidget() {
  const [w, setW] = useState<{ temp: number; text: string; icon: string; mock: boolean } | null>(null)
  ...
  useEffect(() => {
    fetch('/api/weather')
      ...
      .then(d => {
        if (d.code === '200' && d.now) {
          setW({ temp: parseInt(d.now.temp), text: d.now.text, icon: d.now.icon, mock: false })
        } else {
  ...
  return (
    <div className="h-full rounded-xl bg-white/40 backdrop-blur-sm border border-white/30 shadow-md p-4 flex items-center justify-center">
      {loading ? (
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-xs text-slate-500">{w.text}{w.mock ? ' (模拟)' : ''}</span>
          </div>
        </div>
      ) : null}
    </div>
